...
pluto_model = 1;
pluto_channel_model = 1;

if pluto_model == 1
    bpp = 22;
    pluto_model_tag = 'QPSK';
    pluto_model_id = 2;
elseif pluto_model == 2
    bpp = 14;
    pluto_model_tag = '16QAM';
    pluto_model_id = 4;
else
    bpp = 1;
    pluto_model_tag = '64QAM';
    pluto_model_id = 6;
end
...
refQAM = wlanReferenceSymbols(pluto_model_tag);
...
img_tiff = imread("img.tiff");
% imwrite(img_tiff, "tx_img.jpg");
% img_jpg = imread("tx_img.jpg");

imagename_1 = sprintf("tx_img.jpg");
imagename_2 = sprintf("tx_img.bpg");
imagename_3 = sprintf("rx_img.bpg");
imagename_4 = sprintf("rx_img.png");

img_module = py.importlib.import_module('mnist_func');
mnist_input = img_module.mnist_input();

% img = imread("tx_img.jpg");
img_w = func_watermark1(img_tiff);
imwrite(img_w, "tx_img.jpg");
bpg_enc(imagename_1, bpp, imagename_2);
imsize = size(img_w);
...
rawData_1 = readRawData("tx_img.bpg");
txImage = rawData_1(:);
...
nonHTcfg.MCS = pluto_model_id; % Modulation: 64QAM Rate: 2/3
...
elseif strcmpi(channel, "GaussianNoise")
    ...
    % rxWaveform = AWGN_test(txWaveform, SNR);

    if pluto_channel_model == 1
        rxWaveform = awgn(txWaveform, SNR, 'measured', 'dB');
    elseif pluto_channel_model == 2
        % rayleighchan = comm.RayleighChannel('Seed', 16);
        ...
        % ricianchan = comm.RicianChannel('Seed', 16);
...
% specTable = getSpectrumData(spectrumScope);
% figure;
% plot(specTable.FrequencyVector{1, 1} / 1e6, specTable.Spectrum{1, 1});
% grid on;
% xlabel('Frequency (MHz)', 'FontName', 'Times New Roman', 'FontSize', 12);
% ylabel('Power spectral density (dBm/Hz)', 'FontName', 'Times New Roman', 'FontSize', 12);
% title('');
% save("data_specTable.mat", "specTable");
...
while (searchOffset + minPktLen) <= rxWaveformLen
    ...
    % if judge_eqSym == 1
    %     judge_eqSym = 0;
    %     eqSym_temp = reshape(eqSym, [], 1);
    %     scatterplot(reshape(eqSym, [], 1));
    %     grid on;
    %     xlabel('In-phase Amplitude', 'FontName', 'Times New Roman', 'FontSize', 12);
    %     ylabel('Quadrature Amplitude', 'FontName', 'Times New Roman', 'FontSize', 12);
    %     title('');
    %     save("data_eqSym.mat", "eqSym_temp");
    % end
...
if ~(isempty(fineTimingOffset) || isempty(pktOffset))
    ...
    saveRawData(decData, imagename_3);
    bpg_dec(imagename_3, imagename_4);
    RxData_temp = imread(imagename_4);

    % Recreate image from received data
    % fprintf('\nConstructing image from received data.\n');
    receivedImage = uint8(reshape(RxData_temp, imsize));
    ...
    func_watermark2(receivedImage);

    result = img_module.mnist_output()
