...
fileTx = 'img_mnist.jpg'; % Image file name
fData_temp = imread(fileTx); % Read image data from file

imwrite(fData_temp, 'img_mnist_QF.jpg', 'jpg', 'Quality', QF);
fData = imread('img_mnist_QF.jpg');
...
rawData_1 = readJpegRawData('img_mnist_QF.jpg');
txImage = rawData_1(:);
...
if ~(isempty(fineTimingOffset) || isempty(pktOffset))
    ...
    saveJpegRawData(decData, 'pluto_mnist_rx.jpg');
    RxData_temp = imread('pluto_mnist_rx.jpg');
    ...
    % received_Image_logical = logical(receivedImage);
    % imshow(received_Image_logical);

    % save("data_test_received.mat", "received_Image_logical")

    imshow(receivedImage);
    title(sprintf('Received Image'));
    % PSNR = psnr(fData_temp, receivedImage)
    % PSNR = PSNRCalc(fData_temp, receivedImage)
    imwrite(receivedImage, 'img_mnist_D.jpg');
end
